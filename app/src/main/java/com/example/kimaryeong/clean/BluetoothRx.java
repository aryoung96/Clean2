...
public class BluetoothRx extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(BluetoothDevice.ACTION_FOUND)) {
            Toast.makeText(context, "Discovering the BluetoothDevice", Toast.LENGTH_LONG).show();
            BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
            if (sDeviceName.equalsIgnoreCase(device.getName())) {
                sDeviceAddress = device.getAddress();
                Toast.makeText(context, "Device" + sDeviceName + " : " + sDeviceAddress, Toast.LENGTH_LONG).show();
            }
        }
    }
}
